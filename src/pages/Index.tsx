...
const Dashboard = () => {
  ...
  // Loja ativa — lê do localStorage e reage ao evento de troca
  const [activeStoreId, setActiveStoreId] = useState<string>(() =>
    localStorage.getItem("cellmanager-active-store-id") || "all"
  );
  ...
  const fetchData = async () => {
    const isAll = !activeStoreId || activeStoreId === "all";
    ...
    const [productsRes, transactionsRes, storesRes, salesRes, osRes, accRes] = await Promise.all([
      ...
      isAll
        ...
        : supabase.from("transactions").select("*").eq("store_id", activeStoreId),
      supabase.from("stores").select("*"),
      isAll
        ? supabase.from("sales").select("*")
        : supabase.from("sales").select("*").eq("store_id", activeStoreId),
      isAll
        ? supabase.from("service_orders").select("id, status, store_id")
        : supabase.from("service_orders").select("id, status, store_id").eq("store_id", activeStoreId),
      isAll
        ? supabase.from("accessories" as any).select("*")
        : supabase.from("accessories" as any).select("*").eq("store_id", activeStoreId),
    ]);
    ...
    // Store chart (sempre todas as lojas para comparativo)
    ...
    setLowStockStores(stores
      .filter(s => isAll ? (storeStockCounts[s.id] || 0) <= 3 : s.id === activeStoreId && (storeStockCounts[s.id] || 0) <= 3)
      .map(s => ({ name: s.name, count: storeStockCounts[s.id] || 0 })));
  ...
  const isFiltered = activeStoreId && activeStoreId !== "all";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            {isFiltered
              ? <>Dados de <span className="font-semibold text-primary">{activeStoreName}</span></>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* Comparativo entre lojas — sempre visível */}
        <Card className="border-border/50 shadow-lg shadow-black/10">
          <CardHeader className="pb-2">
